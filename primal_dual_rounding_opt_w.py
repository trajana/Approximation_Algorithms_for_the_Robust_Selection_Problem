...
def solve_two_branches_smallest_wi(costs, n, p, k, debug=False):
    ...
    # ---------- Branch wählen: schlechere Garantie (kleineres t) ----------

    # kleinere t => schlechtere Garantie (größeres 1/t)
    choose_in = (t_in <= t_out)
    ...
    if debug:
        ...
        approx_chosen = (1.0 / t_chosen) if t_chosen > 0 else float("inf")
        print("\n=== Two-Branch Smallest-w_i (Shift-Trick) ===")
        print(f"Baseline: t0={t0}  approx={approx0}")
        print(f"i* = {i_star} with w0[i*] = {w_i_star}")
        print(f"Branch IN : t_in={t_in}  approx={approx_in}  |S_in|={len(S_in)}  S_in={S_in}")
        print(f"Branch OUT: t_out={t_out} approx={approx_out} |S_out|={len(S_out)} S_out={S_out}")
        print(f"Chosen branch = {branch} with t={t_chosen}  approx={approx_chosen}")
...
def solve_two_branches_biggest_wi(costs, n, p, k, debug=False):
    ...
    # ---------- schlechtere Garantie wählen (kleineres t) ----------
    choose_in = (t_in <= t_out)
    if choose_in:
        chosen = S_in
        t_chosen = t_in
    ...
    # x-Vektor und robuster Wert (mit ORIGINAL-Kosten)
    x = [1 if i in chosen else 0 for i in range(1, n + 1)]
    obj = robust_obj_of_set(chosen)
